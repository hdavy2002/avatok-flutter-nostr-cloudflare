...
## 📱 LOCAL EMULATOR + HOT RELOAD — how to actually test a fix (2026-08-01)

There IS a working Android emulator with a live Flutter debug session on this Mac.
If you concluded "no emulator / no Flutter session detected", you almost certainly
just had the wrong PATH — `flutter` and `adb` are NOT on the default PATH.

**Always export PATH first. This is the #1 cause of false "no device" reports:**
...
**Check before you claim anything is missing:**

```bash
adb devices                                  # expect: emulator-5554   device
pgrep -f "flutter_tools.snapshot run"        # non-empty = debug session is live
```

**Then use the driver script — do NOT invent your own flutter commands:**

```bash
cd /Users/davy/Documents/websites/avaTOK-2-Flutter
./scripts/dev-emulator.sh            # boot emulator + run app (if nothing is running)
./scripts/dev-emulator.sh reload     # hot reload   — Dart changes, keeps app state (~200ms-1s)
./scripts/dev-emulator.sh restart    # hot restart  — full Dart restart, resets state (~5-10s)
./scripts/dev-emulator.sh log        # tail the run log
```

Hot reload works by sending SIGUSR1 to the `flutter_tools.snapshot run` **dart**
process (SIGUSR2 = restart). Do not signal the wrapper shell — that kills the session.

### WHEN HOT RELOAD DOES NOTHING — read this before reporting a problem

`Reloaded 0 libraries` is NOT a failure. It means no Dart changed. Match the change
to the right action:

| What you changed | What actually makes it visible |
|---|---|
| `app/lib/**` (Dart) | `./scripts/dev-emulator.sh reload` |
| Dart + you want fresh app state | `./scripts/dev-emulator.sh restart` |
| `worker/**` (backend/TypeScript) | **A WORKER DEPLOY. Hot reload does nothing.** |
| `app/android/**`, `pubspec.yaml`, native/plugin code | Full rebuild — `./scripts/dev-emulator.sh` again |

**A backend-only change can never be hot reloaded.** The Flutter app is a client; it
calls the deployed Worker. Editing `worker/src/**` and then asking for a hot reload
will always report 0 libraries and change nothing on screen. Deploy the Worker
instead (and respect the staging/prod rules at the top of this file).

Local runs are DEBUG builds and show `build 28` (from `pubspec.yaml`), not the CI
`10466`-style number — that difference is expected and is not a sign of stale code.
The About screen's `Build (git)` shows the real commit (e.g. `bc6bd9d2-dirty`);
that is the field to trust when checking what is running.
...
<!-- pre-push hook fallback marker — DO NOT REMOVE. The git pre-push hook greps
     AGENTS.md for a quoted group id when ~/.graphiti-projects.tsv lacks an entry,
