...
/// AvaExplore — the Nostr creator marketplace home.
...
class _ExploreHomeState extends State<ExploreHome> {
            ...
            SliverPadding(
              ...
              sliver: SliverGrid(
                ...
                delegate: SliverChildBuilderDelegate(
                  (c, i) => _ProductCard(product: kProducts[i]),
  ...
  Widget _topBar() => Padding(
        ...
        child: Row(
          children: [
            const Icon(Icons.menu, size: 24, color: AvaColors.ink),
            ...
            const Spacer(),
            const Icon(Icons.search, size: 22, color: AvaColors.ink),
            const SizedBox(width: 16),
            _iconBadge(Icons.notifications_none_rounded, dot: true),
            const SizedBox(width: 16),
            _iconBadge(Icons.mail_outline, count: '5'),
          ],
        ),
      );

  Widget _iconBadge(IconData icon, {bool dot = false, String? count}) => Stack(
        clipBehavior: Clip.none,
        children: [
          Icon(icon, size: 22, color: AvaColors.ink),
          if (dot)
            Positioned(right: -1, top: -1, child: _badgeDot()),
          if (count != null)
            Positioned(right: -7, top: -6, child: _badgeCount(count)),
        ],
      );

  Widget _badgeDot() => Container(
      width: 8, height: 8,
      decoration: const BoxDecoration(color: AvaColors.coral, shape: BoxShape.circle));

  Widget _badgeCount(String c) => Container(
  ...
  Widget _search() => Padding(
        ...
        child: Container(
          ...
          child: const Row(children: [
            Icon(Icons.search, color: AvaColors.sub, size: 20),
            SizedBox(width: 10),
            Text('Search presets, courses, services…',
          ...
          itemBuilder: (c, i) {
            ...
            return GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: on ? AvaColors.ink : AvaColors.soft,
                  borderRadius: BorderRadius.circular(20),
                ),
                child: Text(_cats[i],
                    style: TextStyle(
                        color: on ? Colors.white : AvaColors.ink,
  ...
  Widget _drop() => Padding(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
              gradient: AvaColors.dropGradient, borderRadius: BorderRadius.circular(20)),
          child: Column(
            ...
            children: [
              ...
              const Text('Sell your work,\npaid instantly over Nostr',
                  style: TextStyle(color: Colors.white, fontSize: 19, height: 1.2,
              ...
              Container(
                ...
                child: const Text('Start selling',
                    style: TextStyle(color: Color(0xFFFF6F6F), fontWeight: FontWeight.w800)),
              ),
            ],
          ),
        ),
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => Navigator.push(context,
          MaterialPageRoute(builder: (_) => ListingDetail(product: product))),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Expanded(
          ...
          Row(children: [
            Container(width: 16, height: 16,
                decoration: BoxDecoration(
                    gradient: AvaColors.thumbGradients[(product.gradient + 2) % 5],
                    shape: BoxShape.circle)),
            const SizedBox(width: 6),
            Expanded(child: Text(product.author,
                maxLines: 1, overflow: TextOverflow.ellipsis,
