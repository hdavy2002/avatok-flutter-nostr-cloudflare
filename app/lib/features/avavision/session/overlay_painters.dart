...
class _SkeletonPainter extends _BasePainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final accent = Paint()
      ...
      ..strokeWidth = 4
      ..strokeCap = StrokeCap.round;
    final dot = Paint()..color = AD.tabCalls..style = PaintingStyle.fill;
...
class _HandMeshPainter extends _BasePainter {
  ...
  void paint(Canvas canvas, Size size) {
    final accent = Paint()
      ...
      ..strokeWidth = 3
      ..strokeCap = StrokeCap.round;
    final dot = Paint()..color = AD.primaryBadge..style = PaintingStyle.fill;
    for (final inst in f.points) {
...
class _FaceMeshPainter extends _BasePainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final dot = Paint()..color = AD.tabCalls.withValues(alpha: .85)..style = PaintingStyle.fill;
    for (final inst in f.points) {
      for (final k in inst) {
        ...
        final tp = TextPainter(
          text: TextSpan(
            ...
            style: const TextStyle(
                color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: .5),
          ),
          textDirection: TextDirection.ltr,
        )..layout();
        final lblRect = Rect.fromLTWH(rect.left, rect.top - tp.height - 2, tp.width, tp.height);
        canvas.drawRect(lblRect, Paint()..color = AD.primaryBadge);
...
class _MaskPainter extends _BasePainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final tint = Paint()..color = AD.tabCalls.withValues(alpha: .35)..style = PaintingStyle.fill;
