...
/// In-app diagnostic log (ring buffer) shown on the Diagnostics page, plus a
/// [sink] that forwards every line elsewhere — we wire it to PostHog (see
/// Analytics.init) so logs stream live to the backend automatically, keyed by
/// the user's npub. No manual upload, no app-owned DB. Reusable by any app —
/// set [app] to the app key.
class AvaLog {
  AvaLog._();
  ...
  /// App key (future apps set their own, e.g. 'avachat', 'avalive').
  String app = 'avatok';
  ...
  /// Newest-first plain text for the Diagnostics page / clipboard.
