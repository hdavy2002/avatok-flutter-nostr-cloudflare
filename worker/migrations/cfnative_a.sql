-- Re-key batch A — rename per-user npub columns to uid (Clerk user id) on the
...
ALTER TABLE user_vault RENAME COLUMN npub TO uid;
ALTER TABLE communities RENAME COLUMN owner_npub TO owner_uid;
ALTER TABLE community_members RENAME COLUMN npub TO uid;
