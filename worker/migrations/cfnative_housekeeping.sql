-- Housekeeping (Nostr deprecated) — drop the dead legacy npub tables on
...
DROP TABLE IF EXISTS clerk_nostr_link;    -- uid IS the account now
...
-- Strikes still in use (moderation consumer) — re-key its bare npub column.
ALTER TABLE account_strikes RENAME COLUMN npub TO uid;
