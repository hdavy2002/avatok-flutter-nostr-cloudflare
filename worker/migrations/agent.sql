...
-- One persona per (npub, app). persona_prompt + looking_for + boundaries + auto_approve.
...
CREATE INDEX IF NOT EXISTS idx_aconv_npub ON agent_conversations(npub, app_name, created_at);
...
CREATE INDEX IF NOT EXISTS idx_inbox_npub ON agent_inbox(npub, created_at);
CREATE INDEX IF NOT EXISTS idx_inbox_status ON agent_inbox(npub, status, created_at);
