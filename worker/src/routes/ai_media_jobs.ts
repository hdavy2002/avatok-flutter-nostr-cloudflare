...
export async function aiMediaVideoSharePage(req: Request, env: Env, token: string): Promise<Response> {
  ...
  // [VIDEO-AUDIT-1 2026-08-17] og:image now points at the share asset route
  // DIRECTLY, matching the song fix made the same day ([SHARE-OG-IMAGE-1]).
  // That fix established two things about this exact crawler path: social
  // crawlers do not consistently execute a /cdn-cgi/image transform wrapped
  // around a Worker route, and the size problem the wrapper was solving is now
  // solved inside the asset route itself (any thumbnail over ~500 KB is served
  // as a resized JPEG, 200, never a 206). Keeping the wrapper here left video
  // previews on the one path the song page was just moved OFF.
  const socialThumbnail = version ? `${thumbnail}?v=${version}` : thumbnail;
...
export async function aiMediaVideoShareAsset(req: Request, env: Env, token: string, asset: "thumbnail" | "video"): Promise<Response> {
  ...
  // [VIDEO-AUDIT-1] HEAD is included deliberately. Crawlers commonly HEAD an
  // og:image first and judge it by content-type/content-length; skipping the
  // transform for HEAD advertised the raw ~2 MB PNG and then served a small
  // JPEG on GET, so the two answers disagreed about the very thing (size) that
  // makes WhatsApp drop a preview.
  if (asset === "thumbnail" && object.size > 500_000 && !new URL(req.url).searchParams.has("raw")) {
    try {
      const rawUrl = new URL(req.url);
      rawUrl.searchParams.set("raw", "1");
      const resized = await fetch(rawUrl.toString(), {
      ...
      if (resized.ok) {
        return new Response(req.method === "HEAD" ? null : resized.body, {
          headers: {
