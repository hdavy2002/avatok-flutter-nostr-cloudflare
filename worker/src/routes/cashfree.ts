...
async function payEnabled(env: Env): Promise<{ ok: true } | { ok: false; reason: string; status: number }> {
  if (MONEY_IN_DISABLED) return { ok: false, reason: "payments_disabled", status: 503 };
  let cfg;
...
export async function cashfreeCreateOrder(req: Request, env: Env): Promise<Response> {
  ...
  // [WEB-GATEWAY-E 2026-09-18] A badged example never gets a Cashfree order —
  // refused before the direct_purchases row or any gateway call.
