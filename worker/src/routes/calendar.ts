...
//   GET    /api/calendar/slots      → my slots (host) or ?host=<uid> public list
...
export async function listSlots(req: Request, env: Env): Promise<Response> {
  const ctx = await requireUser(req, env);
  if (isFail(ctx)) return json({ error: ctx.error }, ctx.status);
  const host = new URL(req.url).searchParams.get("host") || ctx.uid;
...
export async function cancelSlot(req: Request, env: Env, slotId: string): Promise<Response> {
  ...
  if (!slot || slot.host_npub !== ctx.uid) return json({ error: "slot not found" }, 404);
...
export async function bookSlot(req: Request, env: Env): Promise<Response> {
  ...
  if (slot.host_npub === ctx.uid) return json({ error: "cannot book your own slot" }, 400);
  ...
  ).bind(ctx.uid, slot.start_at, slot.end_at).first<{ x: number }>();
  ...
  if (price > 0) {
    const t = await transferCoins(env, ctx.uid, slot.host_npub, price, APP, `booking:${slotId}`, 0);
  ...
  ).bind(crypto.randomUUID(), bookingId, slotId, owner, role, slot.host_npub, ctx.uid, slot.title, slot.start_at, slot.end_at, price, price > 0 ? 1 : 0, isAgent ? "agent" : "user", now);
...
export async function cancelBooking(req: Request, env: Env): Promise<Response> {
  ...
  if (!list.some((r) => r.owner_npub === ctx.uid)) return json({ error: "not your booking" }, 403);
  ...
  track(env, ctx.uid, "calendar_cancelled", APP, {});
