...
export async function runAgentLoop(
    ...
    ? {
        ...
        parameters: {
          ...
          properties: {
            ...
            duration_seconds: { type: "number", description: "Clip length in seconds. This product generates fixed 10-second clips; omit this field or use 10." },
  ...
  const sys =
    ...
    + (videoDecl
      ...
      : "")
    + (musicDecl
      ? "MUSIC (no lyrics): when the user explicitly asks for MUSIC, a beat, or an instrumental track with NO singing/vocals, first discuss the musical direction if it is missing: genre, mood, energy, setting, instruments, tempo, and what the user intends to do with it. Offer a few fitting options with tiny explanations and ask which they prefer, then call generate_music with the complete brief. Never draft or offer lyrics for an instrumental. "
        + (lyricsDecl
