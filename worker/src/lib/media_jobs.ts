// Durable job state machine for Vertex video/music generation.
//
...
// union to add 'video_generate'/'music_generate' would mean
// editing worker/src/lib/ai_media_jobs.ts, which is a different agent's file
// ownership this wave. This
// module is the coherent alternative: its own D1 table
// (historical media-job migrations, table
// `media_jobs`, DB_MEDIA binding — same database as ai_media_jobs, so a
...
// inline, at submission time (lib/media.ts's runVertexVideo/
// runVertexMusic) to call Venice's queue endpoint; only the returned
// `provider_operation_id` — never the prompt — is persisted, so a redelivered POLL
...
export type MediaJobKind = "video_generate" | "music_generate";
export type MediaJobStatus = "submitting" | "polling" | "delivering" | "succeeded" | "failed" | "cancelled";

export const MODALITY_BY_MEDIA_KIND: Record<MediaJobKind, AiModality> = {
...
export interface MediaJobRecord {
  ...
  // plain TEXT with no CHECK constraint, so the value is
...
function rowToRecord(r: any): MediaJobRecord {
  return {
    job_id: r.job_id, owner_uid: r.owner_uid, conv_id: r.conv_id,
    kind: r.kind, status: r.status,
    provider_operation_id: r.provider_operation_id ?? null,
...
export async function getMediaJob(env: Env, jobId: string): Promise<MediaJobRecord | null> {
  try {
    const r = await env.DB_MEDIA.prepare("SELECT * FROM media_jobs WHERE job_id=?1").bind(jobId).first<any>();
    return r ? rowToRecord(r) : null;
  } catch (e) {
    void trackException(env, e, { route: "media_jobs.getMediaJob", handled: true, extra: { job_id: jobId } });
    return null;
  }
}
...
export async function listMediaJobs(
  ...
  try {
    const rows = await env.DB_MEDIA.prepare(
      "SELECT * FROM media_jobs WHERE owner_uid=?1 AND conv_id=?2 ORDER BY created_at DESC LIMIT ?3",
    ).bind(ownerUid, convId, Math.max(1, Math.min(100, limit))).all<any>();
    return (rows.results ?? []).map(rowToRecord);
  } catch (e) {
    void trackException(env, e, {
      route: "media_jobs.listMediaJobs", handled: true,
...
export async function listMediaJobsForRecovery(
  env: Env,
  nowMs: number,
  ...
  try {
    ...
    void trackException(env, e, {
      route: "media_jobs.listMediaJobsForRecovery", handled: true,
    });
    return [];
  }
}
...
export async function listVideoThumbnailJobsForRecovery(
  ...
  try {
    ...
    const rows = await env.DB_MEDIA.prepare(
      `SELECT * FROM media_jobs
       WHERE kind IN ('video_generate','music_generate')
    ...
    void trackException(env, e, { route: "media_jobs.listVideoThumbnailJobsForRecovery", handled: true });
    return [];
  }
}
...
export async function reopenVideoCover(env: Env, jobId: string): Promise<boolean> {
  try {
    const res = await env.DB_MEDIA.prepare(
      "UPDATE media_jobs SET cover_status='pending', updated_at=?2 WHERE job_id=?1 AND kind IN ('video_generate','music_generate') AND cover_status='failed'",
    ).bind(jobId, now()).run();
    return (res.meta?.changes ?? 0) > 0;
  } catch (e) {
    void trackException(env, e, { route: "media_jobs.reopenVideoCover", handled: true, extra: { job_id: jobId } });
    return false;
  }
}
...
export async function claimMediaRecoveryLease(
  env: Env,
  jobId: string,
  staleBeforeMs: number,
  leasedAtMs: number,
...
export interface CreateMediaJobInput {
  ...
  email?: string | null;
  jobId?: string;
  // Flat tariff in tokens (configured by the caller). When present, reserveAiJob()
...
export type CreateMediaJobResult =
  | { ok: true; job: MediaJobRecord; reservation: ReserveAiJobResult }
 ...
 */
export async function createMediaJob(env: Env, input: CreateMediaJobInput): Promise<CreateMediaJobResult> {
  ...
  const reservation = await reserveAiJob(env, {
    ...
    modality: MODALITY_BY_MEDIA_KIND[input.kind], model: input.model,
  ...
  try {
    await env.DB_MEDIA.prepare(
      `INSERT INTO media_jobs
         (job_id, owner_uid, conv_id, kind, status, provider_operation_id, is_private, tier, has_source_image,
      ...
      input.kind === "music_generate" ? (input.musicMode ?? null) : null,
      ...
      input.kind === "music_generate" || input.kind === "video_generate" ? "pending" : "not_applicable",
      input.deadlineMs, ts,
    ).run();
  } catch (e) {
    ...
    void trackException(env, e, { uid: ownerUid, route: "media_jobs.createMediaJob", handled: true, extra: { job_id: jobId, kind: input.kind } });
    return { ok: false, error: "internal_error" };
  }

  void track(env, ownerUid, "media_job_created", "avaai", {
...
export async function attachProviderOperationId(env: Env, jobId: string, providerOperationId: string): Promise<boolean> {
  try {
    const res = await env.DB_MEDIA.prepare(
      "UPDATE media_jobs SET status='polling', provider_operation_id=?2, updated_at=?3 WHERE job_id=?1 AND status='submitting'",
    ).bind(jobId, providerOperationId, now()).run();
    return (res.meta?.changes ?? 0) > 0;
  } catch (e) {
    void trackException(env, e, { route: "media_jobs.attachProviderOperationId", handled: true, extra: { job_id: jobId } });
    return false;
  }
}
...
export async function bumpMediaJobAttempt(env: Env, jobId: string): Promise<number> {
  try {
    await env.DB_MEDIA.prepare(
      "UPDATE media_jobs SET attempts=attempts+1, updated_at=?2 WHERE job_id=?1 AND status='polling'",
    ).bind(jobId, now()).run();
    const row = await env.DB_MEDIA.prepare("SELECT attempts FROM media_jobs WHERE job_id=?1").bind(jobId).first<{ attempts: number }>();
...
export async function claimMediaDelivery(env: Env, jobId: string, artifactMediaId: string): Promise<boolean> {
  const res = await env.DB_MEDIA.prepare(
    "UPDATE media_jobs SET status='delivering', artifact_media_id=?2, updated_at=?3 WHERE job_id=?1 AND status='polling'",
...
export async function claimMediaDeliveryNotification(env: Env, jobId: string): Promise<boolean> {
  const ts = now();
  const res = await env.DB_MEDIA.prepare(
    `UPDATE media_jobs SET delivery_status='sending', delivery_lease_at=?2, updated_at=?2
...
export async function finishMediaDeliveryNotification(env: Env, jobId: string, sent: boolean): Promise<void> {
  await env.DB_MEDIA.prepare(
    "UPDATE media_jobs SET delivery_status=?2, delivery_lease_at=NULL, updated_at=?3 WHERE job_id=?1 AND status='succeeded' AND delivery_status='sending'",
...
export async function claimSongCover(env: Env, jobId: string): Promise<boolean> {
  try {
    const ts = now();
    const res = await env.DB_MEDIA.prepare(
      "UPDATE media_jobs SET cover_status='generating', updated_at=?2 WHERE job_id=?1 AND kind IN ('music_generate','video_generate') AND (cover_status='pending' OR (cover_status='generating' AND updated_at<?3))",
    ).bind(jobId, ts, ts - 120_000).run();
    return (res.meta?.changes ?? 0) > 0;
  } catch (e) {
    void trackException(env, e, { route: "media_jobs.claimSongCover", handled: true, extra: { job_id: jobId } });
    return false;
  }
}
...
export async function finishSongCover(env: Env, jobId: string, coverMediaId: string | null): Promise<void> {
  ...
  await env.DB_MEDIA.prepare(
    "UPDATE media_jobs SET cover_status=?2, cover_media_id=?3, updated_at=?4 WHERE job_id=?1 AND kind IN ('music_generate','video_generate')",
  ).bind(jobId, status, coverMediaId, now()).run();
}
export type CompleteMediaJobResult = { ok: true; job: MediaJobRecord } | { ok: false; error: string };
...
export async function completeMediaJob(env: Env, input: CompleteMediaJobInput): Promise<CompleteMediaJobResult> {
  const job = await getMediaJob(env, input.jobId);
  ...
  if (job.reservation_id) {
    ...
    const settled = await settleAiJob(env, reservation, {
      opId: input.jobId, uid: job.owner_uid, capability: job.capability, modality: MODALITY_BY_MEDIA_KIND[job.kind],
      ...
      void trackException(env, e, { uid: job.owner_uid, route: "media_jobs.completeMediaJob", method: "settleAiJob", handled: true, extra: { job_id: input.jobId } });
      return null;
    });
  ...
  const upd = await env.DB_MEDIA.prepare(
    "UPDATE media_jobs SET status='succeeded', artifact_media_id=?2, completed_at=?3, updated_at=?3 WHERE job_id=?1 AND status='delivering'",
  ).bind(input.jobId, input.artifactMediaId, ts).run();
  if ((upd.meta?.changes ?? 0) === 0) {
    const cur = await getMediaJob(env, input.jobId);
    if (cur?.status === "succeeded") return { ok: true, job: cur };
    return { ok: false, error: cur ? `invalid_state:${cur.status}` : "not_found" };
  }

  void track(env, job.owner_uid, "media_job_completed", "avaai", { job_id: input.jobId, kind: job.kind, status: "succeeded" });
  const finalRow = await getMediaJob(env, input.jobId);
...
export type FailMediaJobResult = { ok: true; job: MediaJobRecord; transitioned: boolean } | { ok: false; error: string };
...
export async function failMediaJob(env: Env, input: FailMediaJobInput): Promise<FailMediaJobResult> {
  const job = await getMediaJob(env, input.jobId);
  if (!job) return { ok: false, error: "not_found" };
  ...
  const upd = await env.DB_MEDIA.prepare(
    "UPDATE media_jobs SET status='failed', error_code=?2, updated_at=?3 WHERE job_id=?1 AND status IN ('submitting','polling','delivering')",
  ...
  if ((upd.meta?.changes ?? 0) > 0 && job.reservation_id) {
    ...
    await releaseAiJob(env, reservation, { uid: job.owner_uid, opId: input.jobId, capability: job.capability, reason: input.reason || errorCode }).catch((e) => {
      void trackException(env, e, { uid: job.owner_uid, route: "media_jobs.failMediaJob", method: "releaseAiJob", handled: true, extra: { job_id: input.jobId } });
    });
  }
  void track(env, job.owner_uid, "media_job_failed", "avaai", { job_id: input.jobId, kind: job.kind, error_code: errorCode, reason: input.reason });
  const finalRow = await getMediaJob(env, input.jobId);
