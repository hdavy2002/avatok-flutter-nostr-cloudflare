...
async function dispatch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (p === "/api/sitemap/listings" && req.method === "GET") return await cached(req, ctx, () => sitemapListings(env, req), 3600);
      if (p === "/api/sitemap/creators" && req.method === "GET") return await cached(req, ctx, () => sitemapCreators(env, req), 3600);
      if (p === "/api/sitemap/manifest" && req.method === "GET") return await cached(req, ctx, () => sitemapManifest(env, req), 3600);
