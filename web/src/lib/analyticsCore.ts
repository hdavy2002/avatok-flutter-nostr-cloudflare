...
/** Ordered trace updates used by the lightweight facade. */
export function setTrace(traceId: string | undefined): void {
  if (traceId) posthog.register({ trace_id: traceId });
  else posthog.unregister('trace_id');
}
