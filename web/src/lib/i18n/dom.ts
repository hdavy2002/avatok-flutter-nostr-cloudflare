...
function apply(root:ParentNode) {
  root.querySelectorAll<HTMLElement>('[data-india-i18n],[data-i18n]').forEach(node=>{
    ...
    const translated=t(node.dataset.i18n||node.dataset.indiaI18n!,source.text);
    node.textContent=translated;
    if(node.dataset.uiSourceLang)node.lang=translated===source.text?node.dataset.uiSourceLang:getLocale();
  });
