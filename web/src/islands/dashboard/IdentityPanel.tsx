  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-center gap-4 rounded-zine border-zine border-ink bg-card p-5 shadow-zine-sm">
        <div className="flex h-14 w-14 items-center justify-center rounded-zine border-zine border-ink bg-lime font-display text-[24px] font-semibold text-ink shadow-zine-xs"><UiText id="web-dashboard.72dfcfb0c470ac25" source="L" />{cur}</div>
        <div>
          <div className="font-display font-semibold text-[20px] text-ink">{cur >= 3 ? uiT("web-dashboard.dfaa20ad1e33b1c8","Verified creator") : cur === 2 ? uiT("web-dashboard.4f7838402f37674e","Verified") : cur === 1 ? uiT("web-dashboard.7c968fb71f50e335","Member") : uiT("web-dashboard.5ed8944a85a9763f","Guest")}</div>
        ...
        {STEPS.map((s) => {
          ...
          return (
            <div key={s.lvl} className={`flex items-center gap-3 rounded-zine border-zine border-ink ${done ? s.tone : 'bg-paper2'} p-4 shadow-zine-sm`}>
              ...
              <span className="font-body font-extrabold text-[14px] text-ink">{uiSource(s.label)}</span>
            </div>
          );
        })}
      </div>
      <p className="font-body font-bold text-[13px] text-inkSoft"><UiText id="web-dashboard.849649ed4d0c63fb" source="Document KYC unlocks creator payouts. The liveness selfie check uses your camera, so it runs in the Saa Thum phone app." /></p>
    </div>
  );
}
