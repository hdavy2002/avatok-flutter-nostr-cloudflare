          ...
          <div className="overflow-hidden rounded-zine border-zine border-ink bg-card shadow-zine-sm">
            {history.map((h, i) => (
              <div key={h.id ?? i} className={`flex items-center gap-3 p-3 ${i ? 'border-t-zine border-ink' : ''}`}>
                ...
                <span className="rounded-full border-zine border-ink bg-paper px-2 py-0.5 font-mono text-[12px] font-bold uppercase text-inkSoft">{h.status ?? '—'}</span>
                <span className="ml-auto font-mono text-[14px] text-inkMute font-bold">{dt(h.created_at)}</span>
              </div>
            ))}
          </div>
        )}
