  ...
  return (
    <Card fillClassName="bg-card" shadow="lg">
      <div className="flex flex-col items-center gap-4 py-2 text-center">
        <h1 className="font-display font-semibold text-[24px] leading-tight text-ink">Call ended</h1>
        <p className="font-body font-bold text-[15px] text-inkSoft">{REASON_COPY[reason] ?? 'The call ended.'}</p>
        ...
        <div className="mt-2 flex w-full flex-col gap-2.5">
          <a href={listingPath({ id: listingId })} className="no-underline">
            <Button variant="lime" fullWidth label={agentName ? `Talk to ${agentName} again` : 'Book again'} />
          </a>
          <a href="/dashboard" className="no-underline">
            <Button variant="ghost" fullWidth label="My bookings" />
          </a>
        </div>
      </div>
    </Card>
