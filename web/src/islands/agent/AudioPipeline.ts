...
const JITTER_LEAD_SEC = 0.12; // ~120 ms lead so chunks never under-run
...
export class AudioPipeline {
  ...
  private ensureOut(): AudioContext {
    if (!this.outCtx) {
      // Match the agent's 24 kHz output so no resampling artifacts on playback.
  ...
  /** Queue a 24 kHz PCM16 chunk from the agent for gapless playback. */
  ...
  /** Drop everything queued — used on a barge-in / interrupt. */
  clearPlayback(): void {
    const ctx = this.outCtx;
    if (!ctx) return;
    // Rebuild the context to instantly silence scheduled buffers.
    try {
      void ctx.close();
    } catch {
      /* ignore */
