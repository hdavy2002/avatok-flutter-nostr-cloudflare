...
// The Worker moves the AvaCoins instantly and broadcasts a banner over the room
...
export function DonateButton({ listingId, auth, requireAuth }: DonateButtonProps) {
  ...
  async function donate() {
    ...
    try {
      ...
      if (e instanceof ApiError && e.status === 402) {
        setNeedsFunds(true);
        setError('Not enough AvaCoins for that amount.');
  ...
  return (
    <>
      ...
      <Sheet open={open} onClose={() => !busy && setOpen(false)} title="Send AvaCoins" dismissable={!busy}>
        {sent ? (
          <div className="flex flex-col items-center gap-2 py-8 text-center">
            <span className="text-[56px] leading-none" style={{ animation: 'zine-pop 0.5s ease-out' }}>✨</span>
            <p className="font-display font-semibold text-[20px] text-ink">Sent {amount} AvaCoins!</p>
          ...
          <div className="space-y-4">
            ...
            {needsFunds ? (
              ...
              >
                {busy ? 'Sending…' : `Send ${amount} AvaCoins`}
              </button>
            )}
          </div>
        )}
