 ...
 *
 * This is a deliberate PORT of `web/src/islands/consult/PreJoin.tsx`, not a
 * rewrite. Per SPEC-2026-09-01 §4.4 it is provider-agnostic: it only does
 * `getUserMedia` preflight, device enumeration, mute/camera toggles and the
 * `NotAllowedError`/`NotFoundError` copy, then hands the chosen MediaStream
 * up via `onReady`. It does not know GetStream exists.
 *
 * The parent (ConsultRoomGS) does NOT publish this raw MediaStream to
 * GetStream — the SDK's own CameraManager/MicrophoneManager acquire their own
 * tracks on `call.join()`. This component's job ends at "permission granted,
 * these are the user's starting mic/cam preferences and device choices" —
...
export interface PreJoinProps {
  ...
  error?: string | null;
  /** Called with the ready local stream + initial mic/cam state + chosen devices. */
  onReady: (stream: MediaStream, micOn: boolean, camOn: boolean, micId: string, camId: string) => void;
}
