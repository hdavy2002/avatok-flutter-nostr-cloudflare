...
function ConsultRoomGSInner({ booking }: { booking: string }) {
  ...
  const bootstrap = useCallback(async () => {
    ...
    try {
      ...
      if (!mountedRef.current || generation !== operationGenerationRef.current) return;
      showRefusal({ ok: false, reason: 'unavailable', status: 0, detail: 'could not reach Saa Thum' });
      return;
    }
    if (!mountedRef.current || generation !== operationGenerationRef.current) return;
    if ('reason' in res) {
  ...
  switch (refusal.reason) {
      ...
      return (
        <Centered>
          <div className="flex w-full max-w-md flex-col items-center gap-5 text-center">
            ...
            <p className="font-body font-bold text-[15px] text-inkSoft"><UiText id="web-consult-gs.10dffb4cde0f93a3" source="Paid 1:1 sessions aren't live on Saa Thum yet. Check back soon." />{" "}</p>
            <a href="/explore" className="no-underline">
              <Button variant="ghost" label={uiT("web-consult-gs.3b73900b8d29f963","Explore")} />
            </a>
          </div>
    ...
    default:
      return (
        <Centered>
          <div className="flex w-full max-w-md flex-col items-center gap-5 text-center">
            <h1 className="font-display font-semibold text-[26px] text-ink"><UiText id="web-consult-gs.b1cac7b807bbb517" source="Couldn't reach Saa Thum" /></h1>
