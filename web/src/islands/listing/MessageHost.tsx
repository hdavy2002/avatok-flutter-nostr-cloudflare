...
// visitor the creator's Saa Thum number and sends them to the app.
//
// Owner decision: "I don't want users to send messages to creators using the
// web. If a user accesses this on the web, just show him the creator's Saa Thum
// number and ask him to message on that. If the user has the Saa Thum app
...
export interface MessageHostProps {
  listingId: string;
  hostName: string;
  /** The creator's Saa Thum number, as it should be READ ALOUD. Null when they
...
export default function MessageHost({ listingId, hostName, hostNumber }: MessageHostProps) {
  ...
  // Saa Thum number. Pointer-only by design (mouse only, guarded below) and
    ...
    return (
      <div style={wrap} data-section="message_host">
        ...
        <p style={{ margin: 0, fontWeight: 700, fontSize: '0.8125rem', color: '#3a3a34' }}>
          {first}{" "}<UiText id="web-listing.33f64ad76fb76bf5" source="has not set up their Saa Thum number yet. Book a seat and you will be able to reach them from the app." />{" "}</p>
  ...
  return (
    <div style={wrap} data-section="message_host">
      <p style={caption}><UiText id="web-listing.b194d92018d60742" source="MESSAGE" />{" "}{first.toUpperCase()}{" "}<UiText id="web-listing.8dabef32222158db" source="ON SAA THUM" /></p>
      ...
      <p style={{ margin: 0, fontWeight: 700, fontSize: '0.8125rem', color: '#3a3a34', lineHeight: 1.5 }}><UiText id="web-listing.958118278d44c163" source="Messages happen in the Saa Thum app. Open it and send" />{" "}{first}{" "}<UiText id="web-listing.58c67ea31bcc77e8" source="a message on this number — neither of you ever sees the other's real phone number." />{" "}</p>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        ...
        ><UiText id="web-listing.539d6a4198b6e1c7" source="OPEN IN SAA THUM" />{" "}</a>
        <a
          href={PLAY_STORE}
          target="_blank"
          rel="noreferrer"
