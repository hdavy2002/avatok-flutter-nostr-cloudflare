...
function Feed({ title, rows, render, empty }: { title: string; rows: any[]; render: (r: any) => React.ReactNode; empty?: string }) {
  return (
    <section>
      <h2 className="mb-2 font-mono font-bold uppercase text-[14px] tracking-[0.1em] text-blueInk">{title} <span className="text-inkMute">({rows.length})</span></h2>
      {rows.length === 0 ? (
        <p className="font-mono text-[14px] text-inkMute font-bold">{empty ?? 'None active.'}</p>
      ) : (
    ...
    <div className="flex flex-col gap-6">
      ...
      <Feed title="Active consults" rows={d.consults} render={(r) => (
        <div className="flex items-center justify-between p-1">
          <div className="flex flex-col"><span className="font-body font-bold text-[14px] text-ink">{r.kind}</span><span className="font-mono text-[13px] text-inkMute font-bold">{r.creator_id} ← {r.buyer_id}</span></div>
          <Pill>{coins(r.price)}</Pill>
        </div>
      )} />
      <Feed title="Voice agent calls" rows={d.voice_calls} render={(r) => (
        <div className="flex items-center justify-between p-1">
          <div className="flex flex-col"><span className="font-body font-bold text-[14px] text-ink">{r.agent_id}</span><span className="font-mono text-[13px] text-inkMute font-bold">caller {r.user_id} · {r.billed_minutes}/{r.limit_minutes}m</span></div>
      ...
      <Feed title="Vision agent calls" rows={d.vision_calls} empty={d.vision_available ? 'None active.' : 'AvaVision: not yet deployed'} render={(r) => (
        <div className="flex items-center justify-between p-1">
          <div className="flex flex-col"><span className="font-body font-bold text-[14px] text-ink">{r.agent_id}</span><span className="font-mono text-[13px] text-inkMute font-bold">caller {r.user_id} · {r.billed_minutes}/{r.limit_minutes}m · score {r.avg_score ?? '—'} · frames {r.frames_streamed} · snaps {r.snapshot_calls}</span></div>
          <Pill>{coins(r.gross_coins)}</Pill>
        </div>
      )} />

      <section>
        <h2 className="mb-2 font-mono font-bold uppercase text-[14px] tracking-[0.1em] text-blueInk">Conference rooms</h2>
        <p className="font-mono text-[14px] text-inkMute font-bold">{d.conference_rooms.count == null ? 'LiveKit room count not wired (≤25 cap) — see glue note.' : `${d.conference_rooms.count} rooms`}</p>
      </section>

      <section>
        <h2 className="mb-2 font-mono font-bold uppercase text-[14px] tracking-[0.1em] text-blueInk">Per-agent slot utilization (X/{d.slot_utilization.cap})</h2>
        {d.slot_utilization.voice.length === 0 ? <p className="font-mono text-[14px] text-inkMute font-bold">No busy agents.</p> : (
      ...
      <span className="font-mono text-[12px] text-inkMute font-bold">updated {new Date(d.ts).toLocaleTimeString()} · auto-refresh 12s</span>
    </div>
  );
}
