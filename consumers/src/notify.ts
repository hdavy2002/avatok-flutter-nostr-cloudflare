...
export async function notifyUser(env: Env, npub: string, n: { type: string; title: string; body?: string; data?: Record<string, unknown> }): Promise<void> {
  try {
    await env.DB_META.prepare(
      "INSERT INTO notifications (id, npub, type, title, body, data, read, created_at) VALUES (?1,?2,?3,?4,?5,?6,0,?7)",
    ).bind(crypto.randomUUID(), npub, n.type, n.title, n.body ?? null, n.data ? JSON.stringify(n.data) : null, Date.now()).run();
  } catch { /* feed best-effort */ }
  try { await handlePush({ kind: "notify", to_npub: npub, fromName: n.title }, env); } catch { /* push best-effort */ }
}
