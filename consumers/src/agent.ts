...
export async function handleAgent(msg: AgentMsg, env: Env): Promise<void> {
  ...
  if (msg.type === "task") {
    ...
    const persona = await env.DB_META.prepare("SELECT auto_approve, enabled FROM agent_personas WHERE npub=?1 AND app_name=?2")
      .bind(msg.npub, msg.app).first<any>().catch(() => null);
    ...
    await env.DB_META.prepare(
      ...
      crypto.randomUUID(), msg.npub, msg.app,
