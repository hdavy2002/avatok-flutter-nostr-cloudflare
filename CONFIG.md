...
- **ICE (STUN+TURN) for 1:1:** `https://avatok-call-signaling.getmystuffme.workers.dev/ice` (Cloudflare Realtime TURN, key `add95c6c…`, token in Worker secrets)
- **Calls/SFU token API (AvaConsult, paid):** `https://avatok-calls.getmystuffme.workers.dev/join` (mints RealtimeKit participant tokens)
