...
4. **Decisions are FINALIZED** (proposal §6 + Rulebook Golden Rules 13–15): universal per-account storage pool (5 GB free, 20 AvaCoins/GB/month from AvaWallet, read-only never-delete on empty wallet); copy = content-addressed shortcut counted once + cached on-device & Cloudflare; AvaBrain default OFF with a global switch + granular per-app consent toggles + per-app guardrail prompts, private content read on-device only; received media synced with Vault-encrypted keys. Build to these — don't re-litigate.
