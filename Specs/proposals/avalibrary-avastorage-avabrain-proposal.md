...
- **AvaBrain consent:** **default OFF**. A **global Settings switch** ("Let AvaBrain learn from my private content") **plus granular per-app/per-capability toggles** (e.g. "read my AvaTok DMs", "keep a tab on my files"). Each app ships **guardrail system prompts** scoped to its function. Private/E2E content is read **on-device only**; only non-reversible derived data (summary/embedding) is sent. Revocable + purgeable; account deletion purges brain data.
