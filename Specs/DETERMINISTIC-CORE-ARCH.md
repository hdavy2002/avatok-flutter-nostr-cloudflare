...
written order, each stage leaving the app deployable.
...
| 14 | Network Brain | **PHASE B — launch blocker** | **v1.3 naming: the CODE identifier is `ConnectivityCoordinator`** (self-explanatory to future engineers); "Network Brain / NetBrain" stays as the docs nickname. Client singleton: OFFLINE→CONNECTING→CONNECTED→DEGRADED→RECOVERING. SyncHub, Outbox, CallSession, uploads, presence SUBSCRIBE; their private reconnect loops become reactions to one state. Kills competing retry storms. |
...
| 17 | **v1.1: DO diagnostics endpoints** | **PHASE B — launch blocker** | Admin-gated `GET /debug/call/{id}` → {state, **last_transition_reason (v1.3: which event caused the current state, e.g. CONNECTED because ICE_CONNECTED, ENDED because MEDIA_FAILED)**, gen, participants+devices, leases, last heartbeat, outstanding ACKs, event timeline}; `GET /debug/inbox/{uid}` → {cursor, last ids, dedup hits, WS state}. "My call died" becomes one request, not a log hunt. |
...
## Guiding principles for ALL future features (v1.3, from final review)
Durable Objects own state. Clients are requesters, not authorities. Every mutation is
idempotent. Everything important is observable. Failures are expected, not exceptional.
If a future feature can't fit these rules, question the feature or the architecture —
never silently break the rules.

## Changelog
- v1.3 (2026-07-05, review round 3 — APPROVED, feature-frozen): event_id declared
  GLOBALLY unique; code name ConnectivityCoordinator (docs nickname NetBrain);
  diagnostics add last_transition_reason; guiding-principles section; implementation
  discipline note (build strictly in written order, each stage deployable).
